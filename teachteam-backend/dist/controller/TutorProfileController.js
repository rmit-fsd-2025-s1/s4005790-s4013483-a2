...
class TutorProfileController {
    constructor() {
        this.tutorProfileRepository = data_source_1.AppDataSource.getRepository(TutorProfile_1.TutorProfile);
    }
    ...
    async getByEmail(request, response) {
        ...
        if (!email)
            return response.status(400).json({ message: "Mising email" });
    ...
    async upsertByEmail(request, response) {
